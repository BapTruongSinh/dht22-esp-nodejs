  ...
  try {
    ...
    switch (msg.type) {
      case 'sensor_update':
        ...
        if (msg.fan !== undefined) fanSwitch.checked = msg.fan === 'ON';
        if (msg.alarm !== undefined) alarmSwitch.checked = msg.alarm === 'ON';
        updateDevices();
        console.log(`[WS] Sensor: ${msg.temp}°C | ${msg.humi}%`);
        break;

      case 'fan_state':
        fanSwitch.checked = msg.state === 'ON';
        updateDevices();
        console.log(`[WS] Fan: ${msg.state}`);
        break;
      ...
      default:
        console.log(`[WS] Lệnh không xác định: ${msg.type}`);
        break;
    }
  } catch (e) {
...
alarmSwitch.addEventListener('change', () => {
  const state = alarmSwitch.checked ? 'ON' : 'OFF';
  if (state === 'OFF') {
    send({ type: 'buzzer_off' });
    console.log(`[WS] Gửi lệnh tắt còi`);
  } else {
    console.log(`[WS] Server không hỗ trợ lệnh bật còi`);
  }
  updateDevices();
});
